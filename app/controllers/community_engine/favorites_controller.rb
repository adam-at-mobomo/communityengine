...
class FavoritesController < BaseController
  ...
  cache_sweeper CommunityEngine::FavoriteSweeper, :only => [:create, :destroy]  
  
  def create
    @favoritable = params[:favoritable_type].classify.constantize.find(params[:favoritable_id])
    @favorite = CommunityEngine::Favorite.new(:ip_address => request.remote_ip, :favoritable => @favoritable )
